...
| Phase | Scope | Result |
|---|---|---|
| A (`baseline.md`) | Characterize the baseline divider and its 4 call sites; no design work | Accepted as complete -- real synthesis + 520,000-transaction cosimulation confirmed the baseline unchanged, 4 candidate directions identified for Phase B |
| B1 (`phase-b1.md`) | Replace `q4_scale`'s constant-divisor `mx/-8.0f` with an exact power-of-two shortcut | **Exact, but a limited mapped-area win**: 2.2M+ differential cases, 0 mismatches, but the real `q4_scale`-level ECP5 cell reduction was only -2.2% -- ABC's technology mapper was already sharing most of the cost between `q4_scale`'s two divider instances in the baseline |
| B2 (`phase-b2.md`) | Replace the remaining variable-divisor `1/d` with an exact iterative (radix-2) divider | **Large area win, high throughput cost**: -96.9% ECP5 cells at `q4_scale`, but full-serialization scheduling (needed because the new divider is multi-cycle) collaterally slowed 3 untouched chains ~1.9-2.6x -- a queueing cost, not a divider-speed cost |
| B3 (`phase-b3.md`, `phase-b3-root-cause.md`) | Decouple the 3 unaffected chains from Q4_0 encode's in-flight status via a bounded completion reorder buffer | **Correct, but REJECT_ARCHITECTURE** (`decision.md`): 0 fails/drops/duplicates/deadlocks across 7.77M+ transaction-checks at every depth tested, but the only depth that helped at all (4) cost MORE ECP5 area (14,959 cells) than the entire divider unit (2,268 cells) it was protecting, for only a 4-5% throughput gain |
| B4 (`phase-b4.md`) | Replace B2's radix-2 divider with an exact radix-4 (2 quotient bits/cycle) divider, keeping B2's own scheduling, no reorder buffer | **PROMOTE_CANDIDATE**: exact parity vs. both the real divider and B2 simultaneously (4,456,685 cases, 0 mismatches), -32.13% cycles/transaction vs. B2 for only +25.0% ECP5 cells at the `q4_scale` integration point -- the strongest result of any phase |

Full per-phase reasoning, numbers, and reproduction instructions are in
each phase's own document; `decision.md` is the running per-phase decision
...
unchanged. See `promotion-audit.md` (file-by-file classification),
`promotion-plan.md` (integration plan), and `promotion-comparison.md`
...
- The multi-variant (`#ifdef`-selected) Verilator testbench harness used to compare these alternatives during research -- not carried into the production test suite (which uses focused, single-target differential tests instead, `promotion-audit.md`'s NEEDS_CLEANUP items).

## Result files

- `results/synthesis.csv`, `results/baseline-synthesis.txt` -- Phase A baseline synthesis.
- `results/b1-comparison.md` -- Phase B1 numeric comparison.
- `results/b2-comparison.md`, `results/b2-differential.json`, `results/b2-full-datapath.json` -- Phase B2.
- `results/b3-comparison.md`, `results/b3-full-datapath.json`, `results/b3-performance.csv`, `results/b3-synthesis.csv` -- Phase B3.
- `results/b4-comparison.md`, `results/b4-differential.json`, `results/b4-full-datapath.json`, `results/b4-synthesis.csv` -- Phase B4.

## Provenance

- Experiment branch: `experiment/fp-divider-pipeline`.
- Final experiment branch commit: `4c22efa5bc29f42579f2ea641dd6c9458dec988c`
  (research record only -- audit docs, no RTL; the branch's own RTL-bearing
  HEAD, `13240a08...`, predates that).
- Clean promotion PR: [#2](https://github.com/kadireren7/membrane/pull/2)
  (`rtl: replace Q4 combinational dividers with exact radix-4 datapath`),
  squash-merged as `f96c695`.
- Production source (`membrane_fp_scale_neg_pow2.sv`,
  `membrane_fp_divider_radix4.sv`, `q4_scale.sv`,
  `membrane_quant_stream_top.sv`) is now in `main`, under `rtl/`.
- Rejected/superseded RTL (B2, B3, and the intermediate B1/B2/B3 wiring)
  is preserved only on `experiment/fp-divider-pipeline`
  (`rtl/experimental/fp_div/`), never merged, branch not deleted.
- To reproduce the full research record, including the DROP'd phases:
  `git checkout experiment/fp-divider-pipeline` at or after `4c22efa`,
  then `scripts/run-exp-fp-divider-001.sh --phase b1|b2|b3|b4 --quick|--full`
  (that script depends on `rtl/experimental/fp_div/` files that exist only
  on that branch, which is why it was not brought into `main`).
