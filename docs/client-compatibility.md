...
|---|---|---|---|---|---|---|---|
| Python `openai` SDK | 3.11.0 (re-validated Mega Phase E, PR E3; was 3.8.0 at PR D7) | ✅ | ✅ | ✅ | ✅ (real, via a normal `model` field change) | Validated across multiple phases (non-streaming: PR A4; streaming: PR B2; PR D7: real `stop`/`tools`/`response_format` checks, real model-switch proof; PR E3: re-run against the post-E1/E2 build — bounded concurrent decode + multi-model residency — with zero SDK-visible change, since neither touches the `/v1/chat/completions`/`/v1/models` wire shape). Found and fixed a real bug at PR D7: `GET /v1/models` was missing `created` (a REQUIRED field in this SDK's `Model` type) — `client.models.list()` genuinely failed pydantic validation before the fix. See `results/runtime-service/validation.json`, `results/background-service/validation.json`, `results/client-compatibility/validation.json`, `results/api-v1-maturity/validation.json`. | VALIDATED_REAL |
| Node.js `openai` SDK (npm) | 7.13.0 (re-validated Mega Phase E, PR E3; was 7.10.0 at PR D7) | ✅ | ✅ | ✅ | not separately re-tested (same server, same endpoint — no Node-specific switch path exists) | Real async-iterator streaming consumption (`for await`), real `APIError.status`/`.error` shape checks, real `stop`-sequence proof. `scripts/client-compat/test-openai-node.mjs`. Re-run at PR E3, same "no wire-shape change" result as the Python SDK row above. | VALIDATED_REAL |
...
| curl / raw HTTP | n/a | ✅ | ✅ | ✅ (SSE) | ✅ | Used throughout this project's own real evidence gathering, including this phase's own real concurrency-bug hunt below — the lowest-level real client, no SDK abstraction to hide a wire-format bug. Mega Phase E, PR E3: also used to real-validate the new `GET /membrane/v1/capabilities` endpoint and the PR E2 pin/unpin admin routes. | VALIDATED_REAL |
