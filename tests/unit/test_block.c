...
{
    membrane_block_t *block = membrane_block_create(7, MEMBRANE_CODEC_RAW);
...
{
    enum { N = 4096 };
    membrane_block_t    *block = membrane_block_create(1, MEMBRANE_CODEC_RLE);
    uint8_t             in[N];
    uint8_t             out[N];
    size_t              out_len;
    ...
    TEST_ASSERT(membrane_block_write(block, in, N) == MEMBRANE_OK, "block write");
    ...
    TEST_ASSERT(membrane_block_read(block, out, N, &out_len) == MEMBRANE_OK, "block read");
...
{
    membrane_block_t    *block = membrane_block_create(2, MEMBRANE_CODEC_RLE);
    uint8_t             out[4];
    size_t              out_len = 99;
    ...
    TEST_ASSERT(membrane_block_read(block, out, sizeof(out), &out_len) == MEMBRANE_OK,
                "reading empty block succeeds");
...
{
    enum { N = 4096 };
    membrane_block_t    *block = membrane_block_create(3, MEMBRANE_CODEC_RLE);
    uint8_t             in[N];
    uint8_t             out[N];
    size_t              out_len;
    ...
    memset(in, 0x77, N);
    TEST_ASSERT(membrane_block_write(block, in, N) == MEMBRANE_OK, "block write");
    /* Flip a stored byte: read must fail the checksum, not crash. */
...
{
    membrane_block_t *block = membrane_block_create(4, MEMBRANE_CODEC_RAW);

    TEST_ASSERT(block != NULL, "block allocates");
    /* An absurd size must fail cleanly with ALLOC_FAILED, not crash. */
    TEST_ASSERT(membrane_block_write(block, (const uint8_t *)"", (size_t)-1)
                    == MEMBRANE_ERR_ALLOC_FAILED,
                "absurd allocation fails cleanly");
    membrane_block_destroy(block);
}

static void test_invalid_args(void)
{
    membrane_block_t    *block = membrane_block_create(5, MEMBRANE_CODEC_RAW);
    uint8_t             buf[16];
    size_t              out_len;
    ...
    TEST_ASSERT(membrane_block_read(block, buf, 16, NULL) == MEMBRANE_ERR_INVALID_ARG,
                "NULL out_len rejected on read");
    TEST_ASSERT(membrane_block_write(block, buf, 16) == MEMBRANE_OK, "block write");
...
{
    enum { N = 64 * 1024 * 1024 };
    membrane_block_t    *block = membrane_block_create(6, MEMBRANE_CODEC_RLE);
    uint8_t             *in = malloc(N);
    uint8_t             *out = malloc(N);
    size_t              out_len;

    TEST_ASSERT(block && in && out, "large buffers allocate");
    /* Mixed content: compressible runs interleaved with random noise. */
    memset(in, 0, N);
    for (size_t off = 0; off < (size_t)N; off += 8192)
        test_fill_random(in + off, 4096, (uint32_t)(off + 1));
    TEST_ASSERT(membrane_block_write(block, in, N) == MEMBRANE_OK, "large write");
    TEST_ASSERT(membrane_block_read(block, out, N, &out_len) == MEMBRANE_OK, "large read");
