...
static void	test_empty_input(void)
{
	...
	out_len = 99;
	back_len = 99;
	TEST_ASSERT(g_raw->compress(NULL, 0, out, sizeof(out), &out_len)
		== MEMBRANE_OK, "raw compress of empty input");
	TEST_ASSERT(out_len == 0, "empty input compresses to zero bytes");
	TEST_ASSERT(g_raw->decompress(NULL, 0, back, sizeof(back), &back_len)
		== MEMBRANE_OK, "raw decompress of empty input");
...
static void	test_one_byte(void)
{
	...
	in = 0x5A;
	TEST_ASSERT(g_raw->compress(&in, 1, out, sizeof(out), &out_len)
		== MEMBRANE_OK, "raw compress one byte");
	TEST_ASSERT(out_len == 1 && out[0] == 0x5A, "raw stores the byte verbatim");
	TEST_ASSERT(g_raw->decompress(out, out_len, back, sizeof(back), &back_len)
		== MEMBRANE_OK, "raw decompress one byte");
...
static void	test_random_data(void)
{
	...
	in = malloc(N);
	out = malloc(g_raw->bound(N));
	back = malloc(N);
	TEST_ASSERT(in && out && back, "test buffers allocate");
	test_fill_random(in, N, 1234);
	TEST_ASSERT(g_raw->compress(in, N, out, g_raw->bound(N), &out_len)
		== MEMBRANE_OK, "raw compress random data");
	TEST_ASSERT(out_len == N, "raw output size equals input size");
	TEST_ASSERT(g_raw->decompress(out, out_len, back, N, &back_len)
		== MEMBRANE_OK, "raw decompress random data");
