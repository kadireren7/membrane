// A synthesizable, multi-cycle, exact iterative FP32 divider -- same
// reference behavior as membrane_fp_divider.sv (the single-cycle wide
// combinational divider), but implemented as an iterative restoring
// divider that produces TWO quotient bits per clock cycle (13-cycle main
// iteration for a 26-bit fixed-point quotient, vs. 26 single-bit-per-
// cycle iterations for the equivalent radix-2 construction). Exists as an
// alternative call-site replacement for membrane_fp_divider.sv at
// variable-divisor sites where a real `in_valid`/`in_ready`/`out_valid`/
// `out_ready` handshake is acceptable in exchange for a much smaller
// synthesized footprint (see q4_scale.sv's `u_div_id`, `id = 1/d`).
//
// ---- what "radix-4" means here, precisely ----
// By definition, a radix-4 divider produces a quotient digit from the set
// {0,1,2,3} (2 bits) per iteration. This module does exactly that -- every
// ITER cycle advances the quotient by exactly 2 bits. The INTERNAL
// implementation technique is deliberately the simplest one that is
// trivially provable correct by construction: each ITER cycle runs TWO
// radix-2 restoring-division steps (shift, compare, conditional-subtract)
// chained COMBINATIONALLY, and registers the result only once. This is a
// standard, textbook way to build a radix-4 restoring divider (as opposed
// to a direct parallel 4-way magnitude comparison against {0, D, 2D, 3D},
// which is the other standard construction, and which typically underlies
// real SRT radix-4 dividers using a redundant digit set and carry-save
// arithmetic for speed). This module is NOT an SRT divider -- no redundant
// digit set, no carry-save arithmetic -- it is a non-redundant, restoring
// radix-4 divider built from two chained radix-2 steps. The real,
// disclosed trade-off of this choice: the combinational path per ITER
// cycle is roughly TWICE as deep as a single radix-2 step's path (two
// chained compare-and-subtract steps instead of one), which is exactly the
// well-known radix-4-vs-radix-2 trade (fewer cycles, longer combinational
// path per cycle). No real Fmax/timing-closure claim is made anywhere in
// this file -- no vendor place-and-route tool has been used to verify
// this design; see experiments/EXP-FPGA-DIV-001/phase-b4.md for the full
// disclosure of what was and wasn't measured.
//
// ---- why this construction is bit-exact by construction, not just by
// empirical testing (the empirical testing, 4.4M+ cases, still happens --
// see rtl/tb/tb_membrane_fp_divider_radix4.cpp -- this is belt AND
// suspenders, not either/or) ----
// Given rem_reg/quot_reg after N radix-2 steps, running two MORE radix-2
// steps produces EXACTLY the same rem_reg/quot_reg as running two separate
// radix-2 cycles would, because the second step's inputs (the shifted
// rem_reg, the same denom_full) are computed purely combinationally from
// the first step's own output within the same cycle -- there is no
// approximation, reordering, or redundant/carry-save digit anywhere in the
// chain. After 13 such paired cycles (26 individual radix-2 steps total),
// quot_reg/rem_reg reach the same values 26 individual single-step cycles
// would, which are themselves bit-identical to membrane_fp_divider.sv's
// quot64/rem64 (that module's own significand division, `((1<<25) *
// {1,mant_a}) / {1,mant_b}`, is exactly what a 26-cycle radix-2 restoring
// division on the same operands converges to). The FIRST step of the
// FIRST ITER cycle keeps the special "no shift" comparison (it compares
// the raw latched {1,mant_a} with no shift, exactly matching
// membrane_fp_divider.sv's num64/den64 relationship at the top 24 bits) --
// every other step, in every other position, uses the standard "shift the
// running remainder left by one, shift in a 0" pattern (the dividend's
// lower bits are all zero by construction).
//
...
// asynchronous unconditional reset) matches membrane_fp_divider.sv's own
// special-case/rounding semantics exactly -- differential-tested, not just
// argued (rtl/tb/tb_membrane_fp_divider_radix4.cpp,
// experiments/EXP-FPGA-DIV-001/results/b4-differential.json: 4,456,685
// cases, 0 mismatches). The rounding tail operates purely on the FINAL
// quot_reg/rem_reg values, which are unaffected by single-step vs.
// paired-step iteration.
module membrane_fp_divider_radix4 #(
...
`ifndef SYNTHESIS
	initial begin
		if (MANT_ITER_WIDTH != 26)
			$error("membrane_fp_divider_radix4: MANT_ITER_WIDTH must be 26 (the only value derived/verified for FP32's 24-bit significand)");
		if (GUARD_BITS != 2)
			$error("membrane_fp_divider_radix4: GUARD_BITS must be 2 (the only value matching membrane_fp_divider.sv's rounding tail)");
		if ((MANT_ITER_WIDTH % 2) != 0)
			$error("membrane_fp_divider_radix4: MANT_ITER_WIDTH must be even (two radix-2 steps per ITER cycle)");
	end
`endif

	// ---- combinational decode of the operands being accepted THIS
	// cycle -- matches membrane_fp_divider.sv's own special-case decode. ----
	...
	// computation" special case as a plain radix-2 divider -- every
	...
	// ---- rounding-tail combinational logic -- operates only on the final
	// quot_reg/rem_reg values, which are unaffected by single-step vs.
	// paired-step iteration. ----
	logic			norm_shift;
	logic	[25:0]	q_norm;
