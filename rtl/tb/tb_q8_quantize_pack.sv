module tb_q8_quantize_pack;
	...
	initial begin
		...
		$readmemh("/tmp/scale_d.txt", d_vec);
		$readmemh("/tmp/scale_id.txt", id_vec);
		$readmemh("/tmp/pack_expected.txt", pack_exp);
		rst_n = 0;
		valid_in = 0;
		in_idx = 0;
		out_idx = 0;
		...
		while (out_idx < N) begin
			if (in_idx < N) begin
				...
				d_f16_in = d_vec[in_idx];
				id_f32_in = id_vec[in_idx];
			end else
				valid_in = 0;
			@(posedge clk);
			#1;
			if (valid_out) begin
				...
				for (int j = 0; j < 34; j++) begin
					...
					if (got_byte !== pack_exp[out_idx * 34 + j])
						mismatch = 1'b1;
				end
				if (mismatch) begin
					if (fails < 10) begin
						...
						$write("  expect:");
						for (int j = 0; j < 34; j++)
							$write(" %02h", pack_exp[out_idx * 34 + j]);
						$write("\n  got:   ");
						for (int j = 0; j < 34; j++)
		...
		if (fails == 0)
			$display("PASS: q8_quantize_pack bit-exact on %0d blocks", N);
		else
