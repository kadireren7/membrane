module tb_fp_divider;
	...
	membrane_fp_divider #(.DELAY(1)) dut (
	...
	initial begin
		if (!$value$plusargs("afile=%s", a_file))
			a_file = "/tmp/fpdiv_a_small.txt";
		if (!$value$plusargs("bfile=%s", b_file))
			b_file = "/tmp/fpdiv_b_small.txt";
		if (!$value$plusargs("rfile=%s", r_file))
			r_file = "/tmp/fpdiv_r_small.txt";
		...
		if (fails == 0)
			$display("PASS: membrane_fp_divider bit-exact on %0d vectors", N);
		else
