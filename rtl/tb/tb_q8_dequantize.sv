module tb_q8_dequantize;
	localparam int N = 20000;
	logic	[7:0]	pack_flat	[0:N * 34 - 1];
	logic	[15:0]	exp_flat	[0:N * 32 - 1];
	logic			clk;
	logic			rst_n;
	logic			valid_in;
	logic	[271:0]	packed_in;
	logic			valid_out;
	logic	[511:0]	x_out;
	int				in_idx;
	int				out_idx;
	...
	initial begin
		$readmemh("/tmp/pack_expected.txt", pack_flat);
		$readmemh("/tmp/dequant_expected.txt", exp_flat);
		rst_n = 0;
		valid_in = 0;
		in_idx = 0;
		out_idx = 0;
		...
		while (out_idx < N) begin
			if (in_idx < N) begin
				valid_in = 1;
				for (int j = 0; j < 34; j++)
					packed_in[j * 8 +: 8] = pack_flat[in_idx * 34 + j];
			end else
				valid_in = 0;
			@(posedge clk);
			#1;
		...
		if (fails == 0)
			$display("PASS: q8_dequantize bit-exact on %0d blocks", N);
		else
