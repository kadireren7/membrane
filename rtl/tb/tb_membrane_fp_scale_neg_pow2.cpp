// Focused component differential test between the real, general-purpose
// rtl/membrane_fp_divider.sv (b_in held at the exact F32 constant -8.0,
// 0xC1000000 -- the actual constant rtl/q4_scale.sv's `u_div_d` instance
// uses) and rtl/membrane_fp_scale_neg_pow2.sv (SHIFT=3), both built from
...
int	main(int argc, char **argv)
{
	...
	g_candidate = new Vmembrane_fp_scale_neg_pow2;
	...
	{
		printf("PASS: membrane_fp_scale_neg_pow2 vs membrane_fp_divider, %lu cases, 0 mismatches\n",
			(unsigned long)g_result.total);
		return 0;
	}
