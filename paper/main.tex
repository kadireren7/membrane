...
%
% Compiles with a plain `pdflatex main.tex` (run twice for the table of
% contents/references to settle) -- uses an inline `thebibliography`
...
\item A runtime-calibrated, hot/warm/cold KV-cache tiering policy over
  ...
  bit-exact against a production reference (ggml) across 100{,}000+
...
ranges from the CXL Consortium's own specification \cite{cxlconsortium2023spec}.
...
appears anywhere in this project. AI tool contribution policy: portions
of this project's code, documentation, and manuscript were produced with
AI-coding-tool assistance (Claude Code) under the sole author's direction
and review -- \emph{[placeholder: final disclosure wording pending venue
selection, see paper/submission-options.md]}. Authorship: sole author,
Kadir Eren Altınta\c{s}; no co-authors.
...
\texttt{scripts/demo.sh} ($\sim$25s quick check), \texttt{paper/build.sh}.
