...
# almost immediately on a Release build; give it enough time to
# survive being run under ASan/TSan too (both add real, substantial
# per-operation overhead -- interceptors on every zlib call and file
# read while verifying/decoding the 431 MiB synthetic trace, plus
# slower malloc), then kill hard (SIGKILL, the worst case -- no
# graceful shutdown at all) to prove the checkpoint/CSV survive an
# ungraceful death, not just a clean exit.
sleep 45
