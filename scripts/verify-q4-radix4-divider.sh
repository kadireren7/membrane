...
# rtl/membrane_quant_stream_top.sv). Promoted from EXP-FPGA-DIV-001
# (experiments/EXP-FPGA-DIV-001/); see promotion-audit.md/promotion-plan.md
# for what was and wasn't brought into production.
#
...
# project's own measured EXP-FPGA-DIV-001 Phase B4 results
# (experiments/EXP-FPGA-DIV-001/results/b4-synthesis.csv,
# results/b4-differential.json), reproduced fresh on this branch (not
# copied blindly) with a generous tolerance -- not brittle exact-cell
# equality, since yosys is not perfectly deterministic run-to-run/version-
# to-version (ABC's own internal ordering can shift cell counts by a
# handful of cells even for byte-identical RTL -- observed directly while
# preparing this script, see experiments/EXP-FPGA-DIV-001/promotion-
# comparison.md). No real hardware Fmax claim is made or gated on here --
# there is no place-and-route tool in this environment (unchanged
# disclosure from every EXP-FPGA-DIV-001 phase).
set -euo pipefail
